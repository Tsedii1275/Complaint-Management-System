...
import { Card, Typography, Button, Tag, Empty, Alert, Form, Input, Checkbox, Space, Row, Col, Tabs, Table, Select } from 'antd';
import { DeleteOutlined, ArrowLeftOutlined, ToolOutlined, PaperClipOutlined, DownloadOutlined, CheckCircleOutlined, SearchOutlined, SyncOutlined } from '@ant-design/icons';
  ...
  const loadTasks = async (showLoading = false) => {
    try {
      ...
      const workUnitTasks = tasksData.filter(task => 
        ...
        (task.name && task.name.includes('Secondary Resolution Review'))
      );
              ...
              <Col xs={24} lg={11}>
                <div style={{
                  ...
                  <div style={{ marginBottom: '16px' }}>
                    <Text style={{ fontSize: '16px', fontWeight: 700, color: '#0f172a' }}>
                      {selectedTask.name && selectedTask.name.includes('Secondary Resolution Review')
                        ? "Secondary Resolution Review & Action"
                        : (user?.fullName ? `${user.fullName}'s Resolution` : "Resolution Action")}
                    </Text>
                  </div>
                  ...
                  <div style={{ borderTop: '1px solid #e2e8f0', paddingTop: '16px' }}>
                    <Form onFinish={handleSubmit} layout="vertical">
                      <Form.Item
                        label={<span style={{ color: '#475569', fontSize: '14px', fontWeight: 500 }}>{selectedTask.name && selectedTask.name.includes('Secondary Resolution Review') ? "Secondary Investigation / Corrective Action Findings" : "Resolution Details"}</span>}
                        required
                      >
                        <Input.TextArea
                          ...
                          rows={5}
                          placeholder={selectedTask.name && selectedTask.name.includes('Secondary Resolution Review') ? "Enter details of manager secondary resolution findings..." : "Enter detailed resolution details..."}
                          style={{ borderRadius: '8px', border: '1px solid #cbd5e1' }}
                        />
                      </Form.Item>

                      <Form.Item
                        label={<span style={{ color: '#475569', fontSize: '14px', fontWeight: 500 }}>{selectedTask.name && selectedTask.name.includes('Secondary Resolution Review') ? "Action Taken (Optional)" : "Action Taken"}</span>}
                        required={!(selectedTask.name && selectedTask.name.includes('Secondary Resolution Review'))}
                      >
                        <Input.TextArea
                          name="actionTaken"
                      ...
                      {!(selectedTask.name && selectedTask.name.includes('Secondary Resolution Review')) && (
                        <Form.Item style={{ marginBottom: '16px' }}>
                          ...
                          >
                            <span style={{ fontWeight: 500, color: '#0f172a' }}>Mark as Sensitive Case</span>
                          </Checkbox>
                          <span style={{ color: '#94a3b8', display: 'block', marginTop: '4px', fontSize: '12px' }}>
                      ...
                      <div style={{ marginTop: '24px' }}>
                        <Button
                          ...
                          style={{
                            ...
                            height: '44px',
                            fontWeight: 600,
                            background: selectedTask.name && selectedTask.name.includes('Secondary Resolution Review') ? '#faad14' : undefined,
                            borderColor: selectedTask.name && selectedTask.name.includes('Secondary Resolution Review') ? '#faad14' : undefined
                          }}
                        >
                          {isSubmitting 
                            ? 'Processing...' 
